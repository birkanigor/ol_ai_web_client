      ...
      <div class="drawer">
        ...
        <div class="drawer-header">
          <div class="drawer-title">
            ...
            <div>
              <div class="drawer-title-text">Customers</div>
              <div class="drawer-subtitle">{{ groupName }}</div>
            </div>
          </div>
        ...
        <div class="add-row">
          <div class="select-wrap">
            <select v-model="selectedCustomerId" class="select" :disabled="adding">
              <option :value="null">— Select a customer to add —</option>
              <option
        ...
        <div class="drawer-body">
          ...
          <div v-else-if="linked.length === 0" class="empty-state">
            <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" stroke-width="1.5">
              <circle cx="24" cy="20" r="8" /><path d="M8 40c0-8.837 7.163-16 16-16s16 7.163 16 16" />
            </svg>
            <p>No customers linked yet.</p>
          </div>
        ...
        <div class="drawer-footer">
          <span class="footer-count">{{ linked.length }} customer{{ linked.length !== 1 ? 's' : '' }} linked</span>
...
import {
  getGroupCustomers, addGroupCustomer, removeGroupCustomer,
  type GroupCustomer,
} from '../../services/groupCustomersService'
import { getCustomers, type Customer } from '../../services/customersService'
import { useToast } from '../../composables/useToast'

const props = defineProps<{ userGroupId: number; groupName: string }>()
const emit  = defineEmits<{ (e: 'close'): void }>()

const toast            = useToast()
const linked           = ref<GroupCustomer[]>([])
...
const linkedIds = computed(() => new Set(linked.value.map(l => l.customer_id)))

const availableCustomers = computed(() =>
  allCustomers.value.filter(c => !linkedIds.value.has(c.id))
)
...
async function load() {
  ...
  try {
    const [linkedData, allData] = await Promise.all([
      getGroupCustomers(props.userGroupId),
      getCustomers(),
    ])
    linked.value       = linkedData
    allCustomers.value = allData
  } catch {
...
async function add() {
  ...
  adding.value = true
  try {
    const item = await addGroupCustomer(props.userGroupId, selectedCustomerId.value)
    const customer = allCustomers.value.find(c => c.id === selectedCustomerId.value)
    linked.value.push({ id: item.id, user_group_id: props.userGroupId, customer_id: selectedCustomerId.value, customer_name: customer?.customer_name ?? '' })
    linked.value.sort((a, b) => a.customer_name.localeCompare(b.customer_name))
    selectedCustomerId.value = null
    toast.success('Customer linked successfully.')
  } catch (err: unknown) {
    const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message
    toast.error(msg ?? 'Failed to link customer.')
  } finally {
    adding.value = false
  }
}
...
.state-row {
  ...
  padding: 28px 20px; color: #6b7280; font-size: 14px;
}
